...
\indent Lastly, my deepest gratitude is expressed to my fellow graduate students. The last two years wouldn't have been possible without their support, help, and encouragement. My time at Vanderbilt has been a wonderful journey, and I'm honored to have shared my time with them.
...
%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[]
\caption{Attained design characteristics from deviation of Admissible II stage design ($p_0$ = 0.10, $p_1$ = 0.25, $\alpha$ = 0.05, $\beta$ = 0.20)}
\small
...
%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[]
\caption{Attained design characteristics from deviation of Admissible II stage design ($p_0$ = 0.30, $p_1$ = 0.45, $\alpha$ = 0.10, $\beta$ = 0.10)}
\small
  \resizebox{\columnwidth}{!}{%
<<echo=FALSE, results = "asis">>=
...
\begin{landscape}
\begin{figure}[]
\caption{Monte Carlo Simulation of Average Type I Error Rates and Power of 20 Two-Stage Designs when Stage I Sample Size Deviates from Planned for Attained Designs ($n_t^{\ast\ast} = n_1^{\ast\ast} + n_2$). Number of Simulations = \Sexpr{n.sim}.}
\centering
...
\begin{landscape}
\begin{figure}[]
\caption{Monte Carlo Simulation of Average Type I Error Rates and Power of 20 Two-Stage Designs when Stage I Sample Size Deviates from Planned for Attained Designs ($n_t^{\ast\ast} = n_t$). Number of Simulations = \Sexpr{n.sim}.}
...
<<echo=FALSE>>=
ggplot(data = df, aes(x=n.diff,y=averageerror, color = design)) + geom_point() + ylab("Ave. of Ave. Type I and Type II Error") + 
  xlab("Deviation from planned sample size") + scale_colour_manual(name="Design", 
                                                                   values = c("Chang Adaptation" = "red",
                                                                                                    "Chang"     = "dodgerblue3",
                                                                                                    "Likelihood"   = "green4"),
