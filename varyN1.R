...
varyN1 <- function(r1   = 7,   rt  = 21, beta = 0.2, alpha = 0.05,
  ...
  for(i in seq(n1-vary, n1+vary, 2)){  
    ...
    table <- rbind(table, changDesCompare(r1   = r1,  rt  = rt, beta = beta, alpha = alpha,
                                                    n1  = n1,  nt = nt, 
                                                    n1a = i, ntaMeth = ntaMeth,
                                                    p0  = p0, p1 = p1, 
                                                    sim = sim)) 
  }
  table <- table[Cs(p0, p1, n1, n, r1, rt, pet0, EN0, Design, n1star, r1star, rtstar, type1Obs, powerObs, pet0star, EN0star)]
      if(char == TRUE){
